...
export default function validateDocument(document, sourceData = {}) {
  ...
  if (!document || typeof document !== "object" || Array.isArray(document)) {
    warnings.push(
      "Document metadata missing or invalid; created default document object",
    );
  }
  ...
  if (!repaired.schemaVersion) {
    ...
    warnings.push(
      `Schema version mismatch: expected ${SCHEMA_VERSION}, got ${repaired.schemaVersion}`,
    );
  }
  ...
  if (
    !repaired.sourceFingerprint ||
    typeof repaired.sourceFingerprint !== "string"
  ) {
